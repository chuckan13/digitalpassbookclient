...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        for (rowView in passesListView) {
            rowView.findViewById<Button>(R.id.view_button).setOnClickListener {
                val displayPassId = passesListView.adapter.getItemId(rowView.id)
                val action = HomeFragmentDirections.actionNavigationHomeToNavigationDisplayPass(displayPassId)
